<script lang="ts">
    import { Datatable, Pagination, RowsPerPage, type State, TableHandler, Th, ThFilter, ThSort } from '@vincjo/datatables/server';
    ...
    const goodRecordsTable = $derived(new ClientTableHandler($goodRows, { rowsPerPage: Number.MAX_VALUE, selectBy: 'email' }));
    ...
    const badRecordsTable = $derived(new ClientTableHandler($badRows, { rowsPerPage: Number.MAX_VALUE, selectBy: 'email' }));
    ...
    async function exportUsers() {
        ...
        if (result.type === 'success' && result.data) {
            ...
            mount(NoticeModal, {
                target: document.body,
                props: { headerText: "Error exporting users", bodyText: "Please try again or contact administrator", show: true }
            });
        }
    }
</script>
...
<div class="col">
    <Datatable basic table={badRecordsTable}>
        ...
        {#snippet footer()}
            ...
            <div class="row ver-bottom" style="padding: var(--sm); width: 50%">
                ...
                <button type="button" class:hidden={$badOutputColumns === 'givenName,familyName,email,affiliation,country,comments,issues'}
                        onclick={() => $badOutputColumns = 'givenName,familyName,email,affiliation,country,comments,issues'}>Reset
                </button>
    ...
    <Datatable basic table={goodRecordsTable}>
        ...
        <table>
            ...
            <tr>
                <Th>
                    <div class="row ver-center" style="white-space: nowrap; gap: 0"><input checked={goodRecordsTable.isAllSelected}
        ...
        {#snippet footer()}
            ...
            <div class="row ver-bottom" style="padding: var(--sm); width: 50%">
                ...
                <button type="button" class:hidden={$goodOutputColumns === 'givenName,familyName,email,affiliation,country,comments'}
